...
# 2026-08-01 定制：加载仓库根 .env（DOUBAO_HEADLESS/DOUBAO_BROWSER_DATA 等）
try:
...
def create_app(
    *,
    ...
    async def lifespan(app: FastAPI):
        ...
        # 2026-08-01 定制：启动时从 .doubao_session.json 注入 cookie，
        # 否则 headless 浏览器永远显示未登录（仅 QR 登录成功后才注入）。
        if not client.is_ready:
            session_file = os.environ.get("DOUBAO_SESSION_FILE", ".doubao_session.json")
            try:
                from doubao2api.session import load_session
                session = load_session(session_file)
                cookies = session.get("cookies", {})
                if cookies:
                    ok = await client.inject_cookies_and_reload(cookies)
                    log.info("Session cookie inject: %s (%d cookies)", "OK" if ok else "FAILED", len(cookies))
                else:
                    log.warning("Session file has no cookies: %s", session_file)
            except Exception as e:
                log.error("Session cookie inject error: %s", e)
        ...
        for msg in messages:
            ...
            for part in msg.content:
                ...
                if part.get("type") == "text":
                    ...
                    ref = part.get("image_url") if part.get("type") == "image_url" else part.get("file_url")
                    if isinstance(ref, str):
                        file_refs.append({"url": ref, "kind": "image" if part.get("type") == "image_url" else "file"})
                    elif isinstance(ref, dict):
                        file_refs.append({**ref, "kind": "image" if part.get("type") == "image_url" else ref.get("kind", "file")})
        ...
        for file_ref in file_refs:
            ...
            if url.startswith("data:"):
                ...
                if name == "file":
                    ...
                    name = f"upload{ext}"
                # 2026-08-01 定制：图片走 upload_image（type=1 图片附件），其他走 upload_file
                if name.lower().endswith((".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp", ".heic")):
                    uploaded = await client.upload_image(image_bytes=file_data, filename=name)
                    files.append({"uri": uploaded["uri"], "name": uploaded["name"], "size": 0, "kind": "image"})
                else:
                    uploaded = await client.upload_file(file_data=file_data, filename=name)
                    files.append({"uri": uploaded["uri"], "name": uploaded["name"], "size": uploaded["size"], "kind": "file"})
                continue
    ...
    async def chat_completions(body: ChatCompletionRequest, request: Request):
        ...
        try:
            if has_tools:
                ...
                # 2026-08-01 定制：图片附件走 chat_completion（type=1），文档走 chat_with_file（type=3）
                images = [f for f in files if f.get("kind") == "image"]
                docs = [f for f in files if f.get("kind") != "image"]
                if images and not docs:
                    result = await _collect_chat_response(
                        client, prompt, use_deep_think,
                        conversation_id=body.conversation_id, bot_id=body.bot_id,
                    ...
                    result = await client.chat_with_file(
                        text=prompt,
                        file_uri=refs,
                        file_name=refs[0]["name"],
                        file_size=refs[0]["size"],
