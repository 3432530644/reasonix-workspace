...
| 情况 | 处理方式 |
|------|---------|
| vault 根目录不存在 | 询问用户路径，不默认猜测 |
| git 状态检查失败（非 git 仓库） | 提示执行 `git init && git add -A && git commit -m "init"`，停止执行 |
| 文件移动时目标路径已存在同名文件 | 询问用户是否覆盖，不改名也不静默跳过 |
| MOC.md 写入时文件被锁定 | 等待 200ms 后重试，最多 3 次；失败则报错并展示写入内容让用户手动处理 |
| 笔记读取时编码异常 | 跳过该笔记，在变更预览中标注"编码异常，跳过" |
| 无新增笔记（增量模式） | 友好告知用户，正常结束 |

## NEVER

- NEVER 修改笔记的 body 内容 — 只移动文件位置和创建/更新 MOC.md
- NEVER 在用户确认前执行任何文件移动或写入操作
- NEVER 强制将笔记归入不合适的主题 — 无法归类时放入 `misc/` 并标注"待整理"
- NEVER 在一轮增量维护中重新归类已归类过的笔记（除非用户明确要求重分类）
- NEVER 删除笔记或目录而不经过 git 追踪 — 所有变更必须可回退
- NEVER 在初次初始化时一次性读取超过 15 篇笔记的全文 — 严格遵守分步策略
- NEVER 在 MOC.md 中写入笔记描述时截断描述而不告知用户 — 截断需标注"…"
