...
1. **NEVER 将分析结果表述为确定性的投资建议** — 本技能基于《股市真规则》方法论提供分析框架，所有结论必须标注「仅供参考，不构成投资建议」，不得使用「买入/卖出」等指令性语言代替「建议关注/建议谨慎」。

2. **NEVER 在缺少最新财报数据的情况下进行完整的 DCF 估值** — DCF 模型高度依赖自由现金流等财务输入。如果没有最新财报（或用户未提供），必须明确标注「基于公开数据估算，请以最新财报为准」，并降低结论的置信度评级。

3. **NEVER 忽略六个危险信号中的任意一项** — 财务健康检查的六个危险信号（现金流衰退、连续非经常性费用、连续收购、CFO/审计师离职、应收账款异常、赊销条件变更）必须逐项排查，不得选择性跳过。

4. **NEVER 使用单一的估值方法得出结论** — 必须至少结合 DCF 估值、同业 PE/PB 对比、历史估值分位三种方法交叉验证。单一方法的偏差可能误导判断。

5. **NEVER 删除或修改报告标准结构中的任何必需章节** — 所有估值分析报告必须遵循「概述→十分钟测试→护城河分析→财务健康检查→成长性分析→管理层评估→估值分析→同业对比→投资决策整合」的标准结构，可临时增加内容但不可删减。
