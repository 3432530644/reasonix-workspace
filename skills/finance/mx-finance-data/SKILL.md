...
1. **NEVER 在代码、日志、提示词或输出文件中硬编码/明文暴露 `EM_API_KEY`** — API Key 必须只通过环境变量 `EM_API_KEY` 传入，禁止写入任何可被持久化存储或分享的文本中。

2. **NEVER 编造或虚构金融数据** — 如果查询返回空值、报错或数据不完整，必须如实告知用户，不得伪造缺失的指标值或凭空杜撰行情数据。可以建议用户缩小查询范围或分批重试。

3. **NEVER 一次性查询超过 500 个实体** — 系统限制单次最多处理 500 个有效实体。超出时应提示用户分批处理，而非自动截断后无声丢失数据。

4. **NEVER 在 `--indicators` 参数中重复写入实体名称** — `--indicators` 只应包含指标和时间范围等信息，实体信息只放在 `--query` 中。重复写入会导致查询逻辑混乱。

5. **NEVER 忽略错误提示继续执行** — 当遇到「请设置 EM_API_KEY 环境变量」或「缺少 --indicators」等错误时，必须先修复问题再重新执行，不得绕过校验或硬编码占位值。
