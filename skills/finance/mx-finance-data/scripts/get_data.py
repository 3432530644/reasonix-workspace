...
def _write_output_files(
    ...
    文件名使用第一个 sheet 名称中的实体名（如基金/股票名称），便于区分。
    返回 (excel_path, md_path)。
    """
    # 从第一个 sheet 名称提取实体名作为文件基础名
    entity_name = "东方财富"
    if tables:
        raw_name = _flatten_value(tables[0].get("sheet_name", "")).strip()
        if raw_name:
            # 常见格式: "富国全球科技互联网股票(QDII)C(022184.OF)的单"
            # 贪婪匹配到最后一个股票/基金代码（如 (022184.OF)、(NVDA.O) 等）
            m = re.match(
                r'^(.+\([A-Z0-9]+(?:\.[A-Z]+)?\))',
                raw_name,
                re.IGNORECASE
    ...
    safe_name = re.sub(r'[\\/:*?"<>|]', '_', entity_name)[:80]
