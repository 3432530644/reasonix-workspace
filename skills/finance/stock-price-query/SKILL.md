...
1. **NEVER 提供投资建议、买卖建议或预测性结论** — 本技能仅提供客观行情数据（价格、涨跌幅、成交量等）。任何涉及"是否该买/卖/持有"的问题必须回复：「本工具仅提供行情数据，不构成投资建议，请自行判断。」

2. **NEVER 在批量查询中静默跳过失败项** — 批量查询时，对成功的结果正常展示，对失败的项目必须逐条提示用户（如"00700 查询失败，请确认代码是否正确"），不能只展示成功项而忽略报错。

3. **NEVER 接受未经安全校验的用户输入** — 所有 `stock_code` 必须通过正则 `^\.?[A-Za-z0-9]{1,10}$` 校验，`market` 必须在白名单（`sh`/`sz`/`hk`/`us`）内。拒绝任何含特殊字符或 Shell 元字符的输入，防止命令注入。

4. **NEVER 对非交易时段的收盘数据进行"最新价"的误导性表述** — 必须明确标注「当前为非交易时段，显示的是最近一次收盘数据」，避免用户误以为是实时价格。

5. **NEVER 在单次调用中查询超过 20 只股票** — 批量查询上限为 20 只，超出时应提示用户分批查询，而非静默截断列表。
