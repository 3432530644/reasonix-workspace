...
1. **NEVER 将 API Key、Token 或任何敏感凭证硬编码在配置文件中** — 所有鉴权信息必须通过环境变量或安全密钥管理服务注入，禁止提交到版本控制系统。

2. **NEVER 在非交易时段使用高频轮询（< 5 分钟间隔）** — 非交易时段按智能频率表执行（收盘后每30分钟，凌晨每1小时），避免对数据源造成不必要的请求压力。

3. **NEVER 将预警信号作为确定性交易指令执行** — 技术指标（均线、RSI、成交量）有滞后性，预警仅是参考信号，不得替代用户自主判断。输出中必须附带免责提示。

4. **NEVER 忽略多条件共振原则进行单一指标报警** — 单一条件触发仅作提醒级，不得升级为警告或紧急级。紧急级必须同时满足至少三个条件（如价格异动 + 放量 + 技术指标确认）。

5. **NEVER 输出风险提示时使用绝对性用语（如"一定涨"、"保证"、"稳赚"）** — 所有分析结论必须使用概率性表述（如"可能反弹"、"建议关注"、"趋势较强"），并明确提醒投资风险。
